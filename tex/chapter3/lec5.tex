...
\[f'_x(a+\theta_1\D x,b)\appr{\D x\to 0} f'_x(a,b)
\implies A \appr{\D x\to 0} 0\]
...
дифференциала ФНП.}
\begin{defn}
Функция $f(x)$, определённая на некотором множестве $D\subseteq\R^n$,
называется непрерывно дифференцируемой на этом множестве, если в каждой 
...
$D\subseteq\R^m$ и непрерывно дифференцируемые в некоторой окрестности 
точки $t_0\in D$.

Пусть также $f(x)$ -- функция, определённая на $E\subseteq\R^n$ и
...
Для $h(t)=f(g(t))$ в $t_0 \in D \subseteq \R^m$ имеем
...
Пусть $\gamma=\sum_{j=1}^{n}\pderiv{f(x_0)}{x_j}\beta_j+\alpha$.
Тогда:
\begin{equation}
...
\item
	...
	Мы получили, что при $\D t \neq 0 \implies \abs{\frac{\D x_k}{\D t}}
	...
	Т.е. $\alpha = o(\abs{\D t})$.
\item
	Аналогично ограничим первую части $\gamma$:
	...
	Т.е. $\sum_{j=1}^{n}\pderiv{f(x_0)}{x_j}\beta_j=o(\abs{\D t})$.
\end{itemize}
