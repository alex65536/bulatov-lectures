...
Также в силу непрерывности производных $f'_x$ и $f'_y$ и ограниченности $\theta_1$ и $\theta_2$ имеем:
...
Для $h(t)=f(g(t))$, где $g(t)=(g_1(t),g_2(t),\dots,g_n(t))$, в $t_0 \in G \subset \R^m$ имеем
...
Пусть $\gamma=\alpha + \beta$, где $\beta =\sum\limits_{j=1}^{n}\pderiv{f(x_0)}{x_j}\beta_j$.
Тогда:
\begin{equation}
...
\end{itemize}
Из двух пунктов выше получаем $\gamma = \alpha + \beta = o(\abs{\D t})$, что в сочетании
с \eqref{l5:dh} даёт доказательство теоремы.
\end{proof}

\begin{crl}[инвариантность формы 1-го дифференциала ФНП]
Для $h(t) = f(g(t))$ в случае непрерывно дифференцируемых $f(x), g(t)$, где $x=g(t)=(g_1(t),g_2(t),\dots,g_n(t)\in D\subset \R^{n}$, а $t=(t_1,t_2,\dots t_m)\in G\subset \R^{m}$ имеем 
\[dh=\sum_{j=1}^m\pderiv{h(t)}{t_j}dt_j=
\sum_{i=1}^n\pderiv{f(x)}{x_i}dx_i,\]
где $dt_j$~--- приращение независимой переменной $t_j, j=\overline{1,m}$, а $dx_i=dg_i(t)$ --
