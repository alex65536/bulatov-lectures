...
\begin{document}
	...
	\begin{proof}
		...
		\[\forall \varepsilon > 0\ \exists\nu = \nu_\varepsilon\ \colon\ \forall n > 
		m \geq \nu_\varepsilon \implies \sum_{k = m + 1}^n |a_k|\leq\varepsilon.\]
		
		Но тогда
		\[\forall \varepsilon > 0\ \exists\nu = \nu_\varepsilon\ \colon\ \forall n > m 
		\geq \nu_\varepsilon \implies \left|\sum_{k = m + 1}^n a_k\right|
		\leq\sum_{k = m + 1}^n |a_k|\leq\varepsilon.\]
...
\end{remark}
	...
	\begin{example}
		...
		\[\left|\frac{\sin n}{n\sqrt{n + 1}}\right|\leq\frac{1}{n\sqrt{n + 1}}\leq\frac{1}{n^{3/2}}.\]
		
		Последний ряд сходится, так как является обобщённым гармоническим рядом с показателем, большим единицы. 
		А значит, по \hyperref[lec26:comp_test_1]{признаку 1\textdegree}, 
		данный ряд сходится абсолютно, а также, 
	...
	\begin{example}
		...
		т.~е. выполнены все условия \href{29:leibnitz}{признака Лейбница}. Значит, ряд сходится.
...
\end{definition}
	...
	\end{thm}
	\begin{proof}
		\encircle{$\Rightarrow$} Пусть ряд $\sum\limits_{k = 1}^\infty |a_k|$ 
		сходится. 
		Но $b_k\leq |a_k|$, $c_k\leq |a_k|$, $\forall k$. 
		Тогда, по \hyperref[lec26:comp_test_1]{признаку 1\textdegree}, 
		сходятся ряды \eqref{29:10}.
		
		\encircle{$\Leftarrow$} Пусть оба ряда \eqref{29:10}. 
		Тогда
		...
		то и исходный ряд сходится по 
		\hyperref[lec26:comp_test_1]{признаку 1\textdegree}.
