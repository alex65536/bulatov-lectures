...
\begin{proof}
    ...
    \end{vmatrix} \cdot (-1)^{k+1} \cdot \pderiv{H(x, u_{k+1})}{u_{k+1}} = 
    (-1)^{k+1} \cdot \pderiv{H(x, u_{k+1})}{u_{k+1}} \cdot I_0,
    \end{gather*}
    причем в силу \eqref{lec_9.num_15} $I_0 \ne 0$. Также по условию теоремы 
    $I(x_0, u_0) \ne 0$. Но раз \[I(x_0, u_0) = (-1)^{k+1} \cdot \pderiv{H(x, 
    u_{k+1})}{u_{k+1}} \cdot I_0 \ne 0,\]
