...
\begin{document}
	...
	D_k$ 
	и $\mes\left(D_i \cap D_j\right) = 0,\; i \ne j$, т.~е. различные части $D_i$ и 
	$D_j$ могут иметь общими лишь, может быть, граничные точки. (\emph{Примечание редактора:} то, что пересечение имеет меру ноль может также означать, что, например, на плоскости общей может быть прямая).
