...
\begin{document}
	...
	\begin{proof}
		...
		прямоугольнику. В начале рассмотрим произвольное разбиение $P_0 = \left\{ z_k \right\}$, $k = 
		...
		\label{lec14:3}
		\end{equation}\\
		Далее, проведя произвольное разбиение $\widetilde{P_0}$ квадрируемого компакта $D \subset 
		...
		j$, т.~е. квадрируемые части разбиения $\widetilde{P_0}$ между собой могут иметь общими 
	...
	\begin{rems}
		\quad
		\begin{enumerate}
			\item По аналогии с 2И, \eqref{lec14:9} на практике записывается в виде повторных 
			интегралов:
			\begin{equation}
			...
			\item Если кубируемый компакт $H \subset \R$ является цилиндроидом вдоль оси $Oz$, 
			т.~е. 
			...
			компакте $D \subset \R^2$, который является проекцией $H$ на плоскость $Oxy$, то тогда для $f 
			\in C\left(H\right)$, как и для 2И получаем представление
			\[\iiint\limits_{H} f \left( x, y, z \right) dx dy dz = \iint\limits_{D} 
			dx dy \int\limits_{p\left(x, y\right)}^{q\left(x, y \right)} f\left(x, y, 
			z\right) dz.\]
			
			\item Если кубируемый компакт $H \subset \R^3$ является цилиндроидом вдоль оси 
	...
	\begin{example}
		...
		\leq \sqrt{z - x^2},\ -\sqrt{z} \leq x \leq \sqrt{z},\ 0 \le z \le 1 \right\}.\]
	...
	Отсюда, в частности, следует, что для диффеоморфизмов \eqref{lec14:13}, \eqref{lec14:14} 
	...
	\begin{thm}[о замене переменных в $n$-кратном интеграле]
		Если для диффеоморфизма \eqref{lec14:11} и функции $h(x), x\in D$ существует непрерывная композиция
		\[
