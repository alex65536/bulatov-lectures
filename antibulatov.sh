...
sed -i -E 's/епрерывна([а-я]+)\b/епрекращающа\1ся/g' **/*.tex
sed -i -E 's/епрерывну([а-я]+)\b/епрекращающу\1ся/g' **/*.tex
sed -i -E 's/епрерывн([оаы])\b/епрекращаем\1/g' **/*.tex
sed -i -E 's/([Сс])оответствующ/\1оответсвующ/g' **/*.tex
sed -i -E 's/([Гг])ладк/\1адк/g' **/*.tex
sed -i -E 's/([Пп])ризнак/\1ризрак/g' **/*.tex
sed -i -E 's/([Сс])тепенн/\1тепн/g' **/*.tex
